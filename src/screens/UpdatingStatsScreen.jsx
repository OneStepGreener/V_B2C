...
const UpdatingStatsScreen = ({ navigation, route }) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      Alert.alert('Submitting', 'Uploading photo and submitting data...');
      
      // Call the API to complete pickup with photo upload
      const result = await PickupController.completeCurrentPickup({
  ...
  const handleNextPickup = async () => {
    // Check if data has been submitted first
    if (!isSubmitted) {
      Alert.alert('Error', 'Please click Submit button before proceeding to next pickup.');
      return;
    }

    try {
      // Use PickupController to complete current pickup and get next pickup data
      const result = await PickupController.completeCurrentPickup({
        weight: weight,
        completedAt: new Date().toISOString(),
        image: uploadedImage
      ...
      if (result.success) {
        if (result.hasNext) {
          ...
          Alert.alert(
            'Pickup Completed!',
            `Moving to pickup ${result.nextPickup.pickupIndex}`,
            [
              {
                text: 'Continue',
                onPress: () => {
                  // Navigate back to PickupStart - it will automatically load fresh data
          ...
          Alert.alert(
            ...
            result.message || 'All pickups have been completed successfully!',
            [
              {
                text: 'Logout',
                onPress: () => {
        ...
        Alert.alert('Error', result.error || 'Failed to complete pickup. Please try again.');
      }
    } catch (error) {
      console.error('Error completing pickup:', error);
      Alert.alert('Error', 'Failed to process pickup completion. Please check your connection and try again.');
    }
  };
  ...
  return (
    <View style={styles.container}>
      ...
      {/* Next Pickup button */}
      <TouchableOpacity style={styles.nextPickupButton} onPress={handleNextPickup}>
        <Text style={styles.nextPickupText}>
          {currentPickup >= 10 ? 'End Pickup' : 'Next Pickup'}
        </Text>
...
const styles = StyleSheet.create({
  ...
  weightInput: {
    ...
    paddingVertical: 12,
    fontSize: 16,
    color: '#000000', // Fixed: Added black text color to make input visible
  },
  submitButton: {
  ...
  nextPickupButton: {
    ...
    borderRadius: 8,
    paddingVertical: 6, // Further decreased padding
  ...
  nextPickupText: {
    color: 'white',
    fontSize: 14, // Further decreased font size
    fontWeight: 'bold',
  },
  arrowIcon: {
    color: 'white',
